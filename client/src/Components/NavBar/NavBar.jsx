...
const NavBar = ({ defaultSelectedMenu, defaultSelectedOption }) => {
    ...
    useEffect(() => {
        if (defaultSelectedMenu && defaultSelectedOption) {
            if (defaultSelectedMenu === 'Platos Tipicos') {
                ...
                const cacheKey = `Salon_Platos Tipicos_${defaultSelectedOption}`;
                if (!cache[cacheKey]) {
                ...
                const cacheKey = `Salon_Bebidas_${defaultSelectedOption}`;
                if (!cache[cacheKey]) {
                    dispatch(getProductsSubCategorias('Salon', 'Bebidas', defaultSelectedOption));
                }
            }
        }
    }, [defaultSelectedMenu, defaultSelectedOption, dispatch, cache]);
    ...
    const handleMenuChange = (menu) => {
        ...
        if (menu === 'Platos Tipicos') {
            ...
            setSelectedBebidaOption('');
            const subcategory = 'Platos Tipicos';
            const cacheKey = `Salon_Platos Tipicos_${subcategory}`;
            if (!cache[cacheKey]) {
            ...
            setSelectedPlatoOption('');
            const subcategory = 'Bebidas sin alcohol';
            const cacheKey = `Salon_Bebidas_${subcategory}`;
            if (!cache[cacheKey]) {
                dispatch(getProductsSubCategorias('Salon', menu, subcategory));
            }
        }
    ...
    const handleMenuClick = (menu) => {
        setSelectedMenu(menu);
        const subcategory = menu === 'Platos Tipicos' ? 'Platos Tipicos' : 'Bebidas sin alcohol';
        const cacheKey = `Salon_${menu}_${subcategory}`;
        if (!cache[cacheKey]) {
            dispatch(getProductsSubCategorias('Salon', menu, subcategory));
        }
    };
    ...
    const handlePlatoOptionClick = (option) => {
        ...
        setSelectedOption(option);
        const cacheKey = `Salon_Platos Tipicos_${option}`;
        if (!cache[cacheKey]) {
    ...
    const handleBebidaOptionClick = (option) => {
        ...
        setSelectedOption(option);
        const cacheKey = `Salon_Bebidas_${option}`;
        if (!cache[cacheKey]) {
