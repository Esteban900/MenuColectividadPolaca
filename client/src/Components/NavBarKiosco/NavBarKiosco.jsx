...
import style from './NavBarKiosco.module.css';
import { getProductsSubCategorias } from "../../redux/actions";

const NavBarKiosco = ({ defaultSelectedMenu, defaultSelectedOption }) => {
    ...
    useEffect(() => {
        if (defaultSelectedMenu === 'Platos Tipicos') {
            ...
            dispatch(getProductsSubCategorias('Kiosco', 'Platos Tipicos', defaultSelectedOption));
            ...
            dispatch(getProductsSubCategorias('Kiosco', 'Bebidas', defaultSelectedOption));
        }
    ...
    const handleMenuChange = (menu) => {
        ...
        if (menu === 'Platos Tipicos') {
            setSelectedPlatoOption('Comida al Paso');
            setSelectedBebidaOption('');
            let subcategory = 'Comida al Paso';
            dispatch(getProductsSubCategorias('Kiosco', menu, subcategory));
    ...
    const handleOptionClick = (option) => {
        ...
        dispatch(getProductsSubCategorias('Kiosco', selectedMenu, option));
    };
//HANDLE PARA PLATOS O BEBIDAS 1° MENU
    const handlePlatoOptionClick = (option) => {
        setSelectedPlatoOption(option);
        setSelectedOption(option);
        dispatch(getProductsSubCategorias('Kiosco', 'Platos Tipicos', option));
    };
// HANDLE PARA BEBIDAS
    const handleBebidaOptionClick = (option) => {
        setSelectedBebidaOption(option);
        setSelectedOption(option);
        dispatch(getProductsSubCategorias('Kiosco', 'Bebidas', option));
    };
